...
if [[ "$(cat /tmp/db2-role_${DB2INST}.txt)" == "STANDARD" ]]; then
  ...
  if [[ "$(cat /tmp/dbaction.txt | tr A-Z a-z)" == "stop" ]]; then
    echo "\"${SCRIPTSDIR}/runasdb2.sh\" stop_db2.sh ${DB2INST}" >> ${STEPSDIR}/STAGE01.DB2STEP01.${HNAME}.STOP-DB2.TODO
  elif [[ "$(cat /tmp/dbaction.txt | tr A-Z a-z)" == "start" ]]; then
    echo "\"${SCRIPTSDIR}/runasdb2.sh\" start_db2.sh ${DB2INST}" >> ${STEPSDIR}/STAGE01.DB2STEP01.${HNAME}.START-DB2.TODO
  fi
...
## This block will run for Primary db servers.
  ...
  if [[ "$(cat /tmp/dbaction.txt | tr A-Z a-z)" == "stop" && "$(cat /tmp/actionon.txt | tr A-Z a-z)" == "primary" ]]; then
    echo "\"${SCRIPTSDIR}/runasdb2.sh\" stop_db2.sh ${DB2INST}" > ${STEPSDIR}/STAGE02.DB2STEP02.${HNAME}.STOP-DB2.TODO
  elif [[ "$(cat /tmp/dbaction.txt | tr A-Z a-z)" == "start" && "$(cat /tmp/actionon.txt | tr A-Z a-z)" == "primary" ]]; then
    echo "\"${SCRIPTSDIR}/runasdb2.sh\" start_db2.sh ${DB2INST}" > ${STEPSDIR}/STAGE02.DB2STEP01.${HNAME}.START-DB2.TODO
  fi
...
## This block will run for Principal Standby servers.
  ...
  if [[ "$(cat /tmp/dbaction.txt | tr A-Z a-z)" == "stop" && "$(cat /tmp/actionon.txt | tr A-Z a-z)" == "primary" ]]; then
    echo "\"${SCRIPTSDIR}/runasdb2.sh\" failover.sh ${DB2INST}" > ${STEPSDIR}/STAGE01.DB2STEP01.${HNAME}.FAILOVER-DB2.TODO
